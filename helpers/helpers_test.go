...
func TestKeyLength(t *testing.T) {
	...
	sm2In, _ := sm2Priv.Public().(*sm2.PublicKey)
	expSm2 := sm2In.Curve.Params().BitSize
	outSm2 := KeyLength(sm2In)
	if expSm2 != outSm2 {
		t.Fatal("KeyLength malfunctioning on sm2 input")
	}
...
func TestHashAlgoString(t *testing.T) {
	if HashAlgoString(x509.SM2WithSM3) != "SM3" {
		t.Fatal("standin")
	}
...
func TestSignatureString(t *testing.T) {
	if SignatureString(x509.SM2WithSM3) != "SM2-with-SM3" {
