...
func (sp *StandardProvider) Generate(algo string, size int) (err error) {
	...
	case "ecdsa":
		...
		sp.internal.priv = priv
	default:
		return errors.New("transport: invalid key algorithm; only SM2, RSA and ECDSA are supported")
	}
...
func (sp *StandardProvider) Load() (err error) {
	...
	case *sm2.PublicKey, *ecdsa.PublicKey:
		if p.Type != "EC PRIVATE KEY" {
			err = errors.New("transport: PEM type " + p.Type + " is invalid for an SM2/ECDSA key")
			return
		}
	default:
