...
import (
	...
	"github.com/hxx258456/cfssl-gm/errors"
...
func whichCurve(curveType string, sz int) elliptic.Curve {
	switch curveType {
	case "sm2":
		return sm2.P256Sm2()
	case "ecdsa":
...
func TestECGeneration(t *testing.T) {
	...
	for _, sz := range []int{256, 384, 521} {
		...
		if eckey.Curve != whichCurve("ecdsa", sz) {
...
func TestSM2Generation(t *testing.T) {
	var eckey *sm2.PrivateKey

	for _, sz := range []int{256, 384, 521} {
		kr := &KeyRequest{"sm2", sz}
		priv, err := kr.Generate()
		if err != nil {
			t.Fatalf("%v", err)
		}
		eckey = priv.(*sm2.PrivateKey)
		if eckey.Curve != whichCurve("sm2", sz) {
			t.Fatal("Generated key has wrong curve.")
		}
		if sa := kr.SigAlgo(); sa != x509.SM2WithSM3 {
			t.Fatal("Invalid signature algorithm!")
		}
	}
}
