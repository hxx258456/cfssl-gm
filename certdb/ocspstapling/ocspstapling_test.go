...
import (
	...
	"github.com/hxx258456/ccgo/xcrypto/ocsp"
	"github.com/hxx258456/cfssl-gm/certdb"
...
func TestStapleSCTList(t *testing.T) {
	...
	issuer, issuerPrivKey, err := makeCert(nil, nil)
	if err != nil {
		t.Fatal(err)
	}

	// responderCert is a certificate for which to make an OCSP response.
	responderCert, _, err := makeCert(issuer, issuerPrivKey)
	if err != nil {
		t.Fatal(err)
	}
...
func makeCert(issuer *x509.Certificate, signPrivKey interface{}) (*x509.Certificate, crypto.Signer, error) {
	...
	der, err := x509.CreateCertificate(rand.Reader, &template, issuer, privKey.Public(), signPrivKey)
	if err != nil {
		return nil, nil, err
	}
